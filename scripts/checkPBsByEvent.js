...
(async ()=>{
  try {
    ...
    const age = process.argv[3] || '13';
    const sex = process.argv[4] || 'M';
    console.log(`Querying swimmer_personal_bests for payload contains {event: "${event}", age: "${age}", sex: "${sex}"}`);
    const payloadMatch = { event: String(event), age: String(age), sex: String(sex) };
    const { data, error } = await supabase
      .from('swimmer_personal_bests')
      .select('time, name, tiref, pb_date, payload')
